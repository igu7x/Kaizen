/**
 * Script para executar a migração de OKRs da DIJUD
...
async function runMigration() {
    ...
    console.log('📥 INSERÇÃO DE DADOS: OKRs da Diretoria DIJUD');
    console.log('='.repeat(70));
    ...
    try {
        ...
        const directorates = ['DIJUD', 'DPE', 'SGJT'];
        ...
        const sqlPath = path.resolve(__dirname, '../sql/migrations/011_insert_dijud_okrs.sql');
        const sqlContent = fs.readFileSync(sqlPath, 'utf8');
        console.log('\n📄 Arquivo SQL carregado');
        
        console.log('\n📝 Inserindo dados da DIJUD...\n');
        ...
        console.log('\n📋 KEY RESULTS DIJUD POR OBJETIVO:');
        console.log('-'.repeat(70));
        const keyResults = await client.query(`
            ...
            WHERE kr.directorate_code = 'DIJUD' AND (kr.is_deleted = FALSE OR kr.is_deleted IS NULL)
        ...
        // Verificar SITUAÇÃO
        console.log('\n📊 DISTRIBUIÇÃO POR SITUAÇÃO (DIJUD):');
        ...
        const dijudCount = keyResults.rows.length;
        
        if (dijudCount >= 20) {
            console.log('✅ DIJUD: 20 Key Results inseridos corretamente');
        } else {
            console.log(`⚠️ DIJUD: Esperado 20 KRs, encontrado ${dijudCount}`);
        }
