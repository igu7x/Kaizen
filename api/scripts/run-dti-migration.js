/**
 * Script para executar a migração de OKRs da DTI
...
async function runMigration() {
    ...
    console.log('📥 INSERÇÃO DE DADOS: OKRs da Diretoria DTI');
    console.log('='.repeat(70));
    ...
    try {
        ...
        const directorates = ['DTI', 'DPE', 'SGJT', 'DIJUD'];
        ...
        const sqlPath = path.resolve(__dirname, '../sql/migrations/012_insert_dti_okrs.sql');
        const sqlContent = fs.readFileSync(sqlPath, 'utf8');
        console.log('\n📄 Arquivo SQL carregado');
        
        console.log('\n📝 Inserindo dados da DTI...\n');
        ...
        // Listar objetivos da DTI
        console.log('\n📋 OBJETIVOS DTI INSERIDOS:');
        console.log('-'.repeat(70));
        const objectives = await client.query(`
            SELECT code, title FROM objectives 
            WHERE directorate_code = 'DTI' AND (is_deleted = FALSE OR is_deleted IS NULL)
            ORDER BY code
        `);
        ...
        const keyResults = await client.query(`
            ...
            WHERE kr.directorate_code = 'DTI' AND (kr.is_deleted = FALSE OR kr.is_deleted IS NULL)
        ...
        const dtiCount = keyResults.rows.length;
        
        if (dtiCount >= 20) {
            console.log('✅ DTI: 20 Key Results inseridos corretamente');
        } else {
            console.log(`⚠️ DTI: Esperado 20 KRs, encontrado ${dtiCount}`);
        }
