/**
 * Script para verificar as reuniões do CGGPIAC inseridas no banco
 */
...
async function verificar() {
  console.log('========================================');
  console.log('VERIFICAÇÃO: CGGPIAC REUNIÕES');
  console.log('(Comitê de IA e Cognição Automatizada)');
  console.log('========================================\n');
  ...
  try {
    ...
    const comiteResult = await client.query(`
      SELECT id, sigla, nome FROM comites WHERE sigla = 'CGGPIAC'
    `);
    
    if (comiteResult.rows.length === 0) {
      console.log('❌ Comitê CGGPIAC não encontrado!');
      return;
    }
    ...
    console.log('📅 REUNIÕES DO CGGPIAC (2025):');
    console.log('-------------------------------------------');
    ...
    const pautasResult = await client.query(`
      SELECT 
        ...
        p.numero_item,
        SUBSTRING(p.descricao, 1, 70) AS descricao
    ...
    console.log('\n========================================');
    if (totalReunioes === 1 && totalPautas === 6) {
      console.log('✅ VERIFICAÇÃO CONCLUÍDA COM SUCESSO!');
      console.log('   1 reunião e 6 itens de pauta inseridos corretamente.');
    } else {
      console.log('⚠️  VERIFICAÇÃO COM DIVERGÊNCIA:');
      console.log(`   Esperado: 1 reunião e 6 itens de pauta`);
