/**
 * Script para verificar as reuniões do CGTIC inseridas no banco
 */
...
async function verificar() {
  console.log('========================================');
  console.log('VERIFICAÇÃO: CGTIC REUNIÕES');
  console.log('========================================\n');
  ...
  try {
    ...
    const comiteResult = await client.query(`
      SELECT id, sigla, nome FROM comites WHERE sigla = 'CGTIC'
    `);
    
    if (comiteResult.rows.length === 0) {
      console.log('❌ Comitê CGTIC não encontrado!');
      return;
    }
    ...
    console.log('📅 REUNIÕES DO CGTIC (2025):');
    console.log('-------------------------------------------');
    reunioesResult.rows.forEach(r => {
      console.log(`   ${r.numero.toString().padStart(2, '0')}. ${r.titulo} | ${r.data} | ${r.status} | ${r.itens_pauta} item(s)`);
    });
    console.log('');

    // 4. Validar totais esperados
    ...
    console.log('========================================');
    if (totalReunioes === 21 && totalPautas === 88) {
      console.log('✅ VERIFICAÇÃO CONCLUÍDA COM SUCESSO!');
      console.log('   Todos os 21 reuniões e 88 itens de pauta foram inseridos corretamente.');
    } else {
      console.log('⚠️  VERIFICAÇÃO COM DIVERGÊNCIA:');
      console.log(`   Esperado: 21 reuniões e 88 itens de pauta`);
